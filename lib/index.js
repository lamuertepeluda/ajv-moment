...
const momentTestsWithArgs = ['isBefore','isSame','isAfter','isSameOrBefore','isSameOrAfter','isBetween'];
/**
...
function inline(it, keyword, schema) {
    ...
    const _validations = validations.map(function (validation) {
        ...
        _validation.value = _validation.value.map(function (val) {
            const _val = { now: val.now, $data: val.$data, manipulate: val.manipulate, format: _validation.format };
        ...
        _validations.forEach(function (validation, i) {
            ...
            validation.value.forEach(function (val, ii) {
                ...
                if (val.now === true) {
                    ...
                    templ += `
                            const ${testVal} = (typeof (${val.value}) === 'undefined' || (${val.value}) === null) ? null : moment(${val.value}, ${val.format ? JSON.stringify(val.format) : '[moment.ISO_8601]'});
                        `;
                }
            ...
            const testArgsStr = isArray(validation.testArgs) ? 
                `,${validation.testArgs.map(ta => typeof ta === 'string' ? JSON.stringify(ta): ta).join(', ')}`
                : '';
            const validationTestArgsStr = testArgsStr ? `(value, ${testArgsStr})`: '';
            testVals = testVals.slice(0, testVals.length - 1) + testArgsStr+']';
            // if d is not a Moment, test is valid
            const testAcceptsArgs = momentTestsWithArgs.includes(validation.test);
            const testCode = testAcceptsArgs? `(moment.isMoment(${testVals}[0])) ? d.${validation.test}.apply(d, ${testVals}) : true`: `d.${validation.test}()`;
            templ += `
                        const dataValues = [${origVals.join(',')}];
                        const ${testResult} = ${testCode};
                        if (!${testResult}) {
                            ...
                            ${testVals}.forEach(function(c, i) {
                                if(moment.isMoment(c))
                                {
                                    ...
                                    const erroVal = dataValue === null ? c.format(d.creationData().format) : dataValue;
